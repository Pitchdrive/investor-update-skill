...
## Step 0 — Continuity first

Look for previous updates in `investor-updates/` (or ask where they live). If one exists:

1. Extract every commitment, target, and "next month we will…" from it.
...
If this is the first update, ask for (or infer from the repo/docs): company one-liner,
stage, last round, and roughly how many investors receive the update.
